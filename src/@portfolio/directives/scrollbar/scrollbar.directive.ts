...
import {
    ScrollbarGeometry,
    ScrollbarPosition,
} from '@portfolio/directives/scrollbar/scrollbar.types';
 ...
 */
@Directive({
    selector: '[portfolioScrollbar]',
    exportAs: 'portfolioScrollbar',
    standalone: true,
})
export class PortfolioScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_portfolioScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    private _elementRef = inject(ElementRef);
    private _platform = inject(Platform);

    @Input() portfolioScrollbar: boolean = true;
    @Input() portfolioScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('portfolioScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.portfolioScrollbar = coerceBooleanProperty(
                changes.portfolioScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.portfolioScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('portfolioScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.portfolioScrollbarOptions.currentValue
            );
     ...
     */
    isEnabled(): boolean {
        return this.portfolioScrollbar;
    }
