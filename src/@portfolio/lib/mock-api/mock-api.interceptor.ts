...
import { PORTFOLIO_MOCK_API_DEFAULT_DELAY } from '@portfolio/lib/mock-api/mock-api.constants';
...
export const mockApiInterceptor = (
    ...
    const defaultDelay = inject(PORTFOLIO_MOCK_API_DEFAULT_DELAY);
    const portfolioMockApiService = inject(PortfolioMockApiService);

    // Try to get the request handler
    const { handler, urlParams } = portfolioMockApiService.findHandler(
