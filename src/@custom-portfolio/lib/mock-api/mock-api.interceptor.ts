...
import { CUSTOM_PORTFOLIO_MOCK_API_DEFAULT_DELAY } from '@custom-portfolio/lib/mock-api/mock-api.constants';
import { CustomPortfolioMockApiService } from '@custom-portfolio/lib/mock-api/mock-api.service';
...
export const mockApiInterceptor = (
    ...
    const defaultDelay = inject(CUSTOM_PORTFOLIO_MOCK_API_DEFAULT_DELAY);
    const custom-portfolioMockApiService = inject(CustomPortfolioMockApiService);

    // Try to get the request handler
    const { handler, urlParams } = custom-portfolioMockApiService.findHandler(
