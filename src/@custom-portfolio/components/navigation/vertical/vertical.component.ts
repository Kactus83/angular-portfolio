...
import {
    CustomPortfolioNavigationItem,
    CustomPortfolioVerticalNavigationAppearance,
    CustomPortfolioVerticalNavigationMode,
    CustomPortfolioVerticalNavigationPosition,
} from '@custom-portfolio/components/navigation/navigation.types';
import { CustomPortfolioVerticalNavigationAsideItemComponent } from '@custom-portfolio/components/navigation/vertical/components/aside/aside.component';
...
import { CustomPortfolioVerticalNavigationGroupItemComponent } from '@custom-portfolio/components/navigation/vertical/components/group/group.component';
...
import { CustomPortfolioScrollbarDirective } from '@custom-portfolio/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector: 'custom-portfolio-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: custom-portfolioAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'custom-portfolioVerticalNavigation',
    standalone: true,
    imports: [
        CustomPortfolioScrollbarDirective,
        CustomPortfolioVerticalNavigationAsideItemComponent,
        ...
        CustomPortfolioVerticalNavigationGroupItemComponent,
        CustomPortfolioVerticalNavigationSpacerItemComponent,
    ],
})
export class CustomPortfolioVerticalNavigationComponent
    implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: CustomPortfolioVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: CustomPortfolioVerticalNavigationMode = 'side';
    @Input() name: string = this._custom-portfolioUtilsService.randomId();
    @Input() navigation: CustomPortfolioNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: CustomPortfolioVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<CustomPortfolioVerticalNavigationAppearance> =
        new EventEmitter<CustomPortfolioVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<CustomPortfolioVerticalNavigationMode> =
        new EventEmitter<CustomPortfolioVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<CustomPortfolioVerticalNavigationPosition> =
        new EventEmitter<CustomPortfolioVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<CustomPortfolioNavigationItem> =
        new ReplaySubject<CustomPortfolioNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<CustomPortfolioNavigationItem> =
        new ReplaySubject<CustomPortfolioNavigationItem>(1);
    ...
    private _custom-portfolioScrollbarDirectives!: QueryList<CustomPortfolioScrollbarDirective>;
    private _custom-portfolioScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'custom-portfolio-vertical-navigation-animations-enabled':
                this._animationsEnabled,
            [`custom-portfolio-vertical-navigation-appearance-${this.appearance}`]: true,
            'custom-portfolio-vertical-navigation-hover': this._hovered,
            'custom-portfolio-vertical-navigation-inner': this.inner,
            'custom-portfolio-vertical-navigation-mode-over': this.mode === 'over',
            'custom-portfolio-vertical-navigation-mode-side': this.mode === 'side',
            'custom-portfolio-vertical-navigation-opened': this.opened,
            'custom-portfolio-vertical-navigation-position-left': this.position === 'left',
            'custom-portfolio-vertical-navigation-position-right':
                this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for custom-portfolioScrollbarDirectives
     */
    @ViewChildren(CustomPortfolioScrollbarDirective)
    set custom-portfolioScrollbarDirectives(
        custom-portfolioScrollbarDirectives: QueryList<CustomPortfolioScrollbarDirective>
    ) {
        // Store the directives
        this._custom-portfolioScrollbarDirectives = custom-portfolioScrollbarDirectives;

        // Return if there are no directives
        if (custom-portfolioScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._custom-portfolioScrollbarDirectivesSubscription) {
            this._custom-portfolioScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._custom-portfolioScrollbarDirectivesSubscription = merge(
            ...
            .subscribe(() => {
                ...
                custom-portfolioScrollbarDirectives.forEach((custom-portfolioScrollbarDirective) => {
                    custom-portfolioScrollbarDirective.update();
                });
            });
    }
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            if (
                ...
                const activeItem =
                    this._navigationContentEl.nativeElement.querySelector(
                        '.custom-portfolio-vertical-navigation-item-active'
                    );
            ...
            // Otherwise
                ...
                this._custom-portfolioScrollbarDirectives.forEach(
                    (custom-portfolioScrollbarDirective) => {
                        // Skip if not enabled
                        if (!custom-portfolioScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        custom-portfolioScrollbarDirective.scrollToElement(
                            '.custom-portfolio-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: CustomPortfolioNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: CustomPortfolioNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('custom-portfolio-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'custom-portfolio-vertical-navigation-overlay-transparent'
            );
        }
    ...
    private _showAsideOverlay(): void {
        ...
        this._asideOverlay.classList.add(
            'custom-portfolio-vertical-navigation-aside-overlay'
        );
