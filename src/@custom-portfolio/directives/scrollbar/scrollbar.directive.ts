...
import {
    ScrollbarGeometry,
    ScrollbarPosition,
} from '@custom-portfolio/directives/scrollbar/scrollbar.types';
 ...
 */
@Directive({
    selector: '[custom-portfolioScrollbar]',
    exportAs: 'custom-portfolioScrollbar',
    standalone: true,
})
export class CustomPortfolioScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_custom-portfolioScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    private _elementRef = inject(ElementRef);
    private _platform = inject(Platform);

    @Input() custom-portfolioScrollbar: boolean = true;
    @Input() custom-portfolioScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('custom-portfolioScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.custom-portfolioScrollbar = coerceBooleanProperty(
                changes.custom-portfolioScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.custom-portfolioScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('custom-portfolioScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.custom-portfolioScrollbarOptions.currentValue
            );
     ...
     */
    isEnabled(): boolean {
        return this.custom-portfolioScrollbar;
    }
    ...
    private _init(): void {
        ...
        if (
            ...
            this.custom-portfolioScrollbar = false;
            return;
        }
